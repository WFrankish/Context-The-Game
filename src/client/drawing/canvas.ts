...
class Canvas {
  ...
  drawImage(
      img: HTMLImageElement, imgX: number, imgY: number, width: number,
      height: number, posX: number, posY: number, scaleX = 1, scaleY = 1) {
    ...
    const imgPosX =
        this._width / 2 + posX * tileWidth - (imgWidth - tileWidth) / 2;

    // draw from bottom
    const imgHeight = height * scaleY;
    const imgPosY = this._height / 2 - posY * tileWidth - imgHeight;

    const offscreen = imgPosY + imgHeight < 0 || imgPosY > this._height ||
        imgPosX + imgWidth < 0 || imgPosX > this._width;

    if (!offscreen) {
      this.ctx.drawImage(
          img, imgX, imgY, width, height, imgPosX, imgPosY, imgWidth,
          imgHeight);
    }
  }
