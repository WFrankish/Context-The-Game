...
export class Transform {
  ...
  static rotate(radians: number): Transform {
    const c = Math.cos(radians),
      s = Math.sin(radians);
    return new Transform(new Vector2(c, s), new Vector2(-s, c), new Vector2(0, 0));
  }
  static scale(factor: number): Transform {
    return new Transform(new Vector2(factor, 0), new Vector2(0, factor), new Vector2(0, 0));
  }
  multiply(t: Transform): Transform {
    return new Transform(this.applyDirection(t.a), this.applyDirection(t.b), this.apply(t.c));
  }
  inverse(): Transform {
    const a = this.a.x,
      b = this.b.x,
